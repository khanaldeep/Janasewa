...
const clothesItemsContainer = document.getElementById('clothes-items-container');

firebase.database().ref('clothe_recycle_buy').on('value', snapshot => {
    clothesItemsContainer.innerHTML = '';
    snapshot.forEach(childSnapshot => {
        ...
        `;
        clothesItemsContainer.appendChild(itemDiv);
    });

    document.querySelectorAll('.buy-button').forEach(button => {
        button.addEventListener('click', (e) => {
            const itemId = e.target.getAttribute('data-id');
            window.location.href = `item_details.html?id=${itemId}&category=clothes`;
        });
    });
});
